...
async function initializeAttendanceDates() {
  ...
  try {
    ...
    const subjectsWithoutAttendanceDates = await collection.find({
      attendanceDates: { $exists: false }
    }).toArray();
    
    console.log(`Found ${subjectsWithoutAttendanceDates.length} subjects without attendanceDates field`);
    ...
    console.log(`Updated ${updateResult.modifiedCount} subjects with empty attendanceDates array`);
