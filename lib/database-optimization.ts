...
export async function createDatabaseIndexes(): Promise<void> {
  ...
  try {
    ...
    for (const collectionDef of indexDefinitions) {
      ...
      console.log(`📊 Creating indexes for collection: ${collectionDef.collection}`);
      ...
      for (const indexDef of collectionDef.indexes) {
        try {
          ...
          console.log(`  ✅ Created index: ${indexDef.options?.name || 'unnamed'} -> ${result}`);
        } catch (error: any) {
          // Index might already exist, that's okay
          if (error.code === 85) { // IndexOptionsConflict
            console.log(`  ⚠️  Index already exists: ${indexDef.options?.name || 'unnamed'}`);
          } else {
            console.error(`  ❌ Failed to create index: ${indexDef.options?.name || 'unnamed'}`, error.message);
          }
        }
      }
    }
...
export class OptimizedQueries {
  ...
  async getUserSubjects(userId: string, options: {
  ...
  async getUserTasks(userId: string, options: {
    ...
    const { status, priority, page = 1, limit = 20, sortBy = 'dueDate' } = options;
